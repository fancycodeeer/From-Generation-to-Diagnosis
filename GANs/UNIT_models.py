...
class MsImageDis(nn.Module):
    ...
    def __init__(self, input_dim, n_layer, gan_type, dim, norm, activ, num_scales, pad_type):
...
class VAEGen(nn.Module):
    ...
    def forward(self, images, mode):
        ...
        if mode == 'encode':
            ...
            images = self.dec(images)
            return images
        #
        # if self.training == True:
        #     noise = Variable(torch.randn(hiddens.size()).cuda(hiddens.data.get_device()))
        #     images_recon = self.decode(hiddens + noise)
        #     return images_recon, hiddens, noise
        # else:
...
class VAEGen_encoder(nn.Module):
    # VAE architecture
    def __init__(self, input_dim, dim, n_downsample, n_res, activ, pad_type):
        super(VAEGen_encoder, self).__init__()
        # content encoder
        self.enc = ContentEncoder(n_downsample, n_res, input_dim, dim, 'in', activ, pad_type=pad_type)
...
class VAEGen_decoder(nn.Module):
    # VAE architecture
    def __init__(self, input_dim, dim, n_downsample, n_res, activ, pad_type):
        super(VAEGen_decoder, self).__init__()
        self.dec = Decoder(n_downsample, n_res, self.enc.output_dim, input_dim, res_norm='in', activ=activ, pad_type=pad_type)

    def forward(self, hidden):
        images = self.dec(hidden)
        return images
