...
dataset = ImageDataset(opt.dataroot, transforms_=transforms_, data='Denoise', get_name=False, unaligned=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # pix2pix loss
        # loss_p2p = criterion_cycle(netG_A2B(real_A), real_B) * 10 + criterion_cycle(netG_B2A(real_B), real_A) * 10
        ...
        # Total loss
        loss_G = loss_GAN_A2B + loss_GAN_B2A + loss_identity_A + loss_identity_B + loss_cycle + loss_freq
        loss_G.backward()
