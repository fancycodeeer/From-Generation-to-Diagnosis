...
from models_CUT import Discriminator
from models_CUT import PatchNCE
...
optimizer_G_A = torch.optim.Adam(netG_A2B.parameters(), lr=opt.lr, betas=(0.5, 0.999))
optimizer_G_B = torch.optim.Adam(netG_B2A.parameters(), lr=opt.lr, betas=(0.5, 0.999))
...
# Make mask_H
mask_H, mask_L = make_mask([opt.batchSize, opt.input_nc, opt.size, int(opt.size / 2 + 1)], D=25)  # 10, 20, 30, 40, 50
freq_weight = 2.0
###################################
...
for epoch in range(opt.epoch, opt.n_epochs):
	...
	for i, batch in enumerate(dataloader):
		...
		# PatchNCE loss
		loss_patch_nce = PatchNCE(netG_B2A, opt.batchSize, real_B, fake_A)  # + PatchNCE(netG_A2B, opt.batchSize, real_A, fake_B)
		...
		loss_freqL = loss_freqL_B2A # + loss_freqL_A2B
		...
		loss_freqH = loss_freqH_B2A # + loss_freqH_A2B

		loss_freq = (loss_freqH + loss_freqL) * freq_weight

		# Total loss
		loss_G = loss_GAN_B2A + loss_identity_A + loss_patch_nce * 7.0 + loss_freq
		loss_G.backward()
		...
		if local_rank == 0:
			...
			logger.log({'loss_G': loss_G,
			            'loss_G_identity': (loss_identity_A ),
			            'loss_G_GAN': (loss_GAN_B2A),
			            'loss_G_NCE': loss_patch_nce,
			            'loss_D': (loss_D_A),
			            'loss_freq': loss_freq,
			            # 'loss_p2p': loss_p2p,
			            }
	...
	if local_rank == 0:
		# torch.save(netG_A2B.state_dict(), './output/' + str(epoch) + '_netG_A2B_CUT' + '.pth')
		torch.save(netG_B2A.state_dict(), './output/' + str(epoch) + '_netG_B2A_CUT' + '.pth')
