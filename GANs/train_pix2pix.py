...
from models_b import Generator
from models_b import Discriminator
...
parser.add_argument('--dataroot', type=str, default='./datasets/Carla_Simulator_Parired_Images/', help='root directory of the dataset')
...
dataset = ImageDataset(opt.dataroot, transforms_=transforms_, data='Carla', get_name=False)
sampler = DistributedSampler(dataset)
dataloader = DataLoader(dataset, batch_size=opt.batchSize, shuffle=(sampler is None), num_workers=opt.n_cpu, drop_last=True,
...
for epoch in range(opt.epoch, opt.n_epochs):
	...
	for i, batch in enumerate(dataloader):
		...
		# pixel loss
		loss_pix_B2A = criterion_pix(fake_A, real_A)
		loss_pix_A2B = criterion_pix(fake_B, real_B)
		...
		# loss_freqL_A2B = criterion_L1(freqL_A2B, fake_B_freq[1]) # + criterion_L1(freqL_A2B, fake_B_freq[1])
		# loss_freqL_B2A = criterion_L1(freqL_B2A, fake_A_freq[1]) # + criterion_L1(freqL_B2A, fake_A_freq[1])
		...
		# loss_freqH_A2B = criterion_L1(freqH_A2B, fake_B_freq[0]) # + criterion_L1(freqH_A2B, fake_B_freq[0])
		# loss_freqH_B2A = criterion_L1(freqH_B2A, fake_A_freq[0]) # + criterion_L1(freqH_B2A, fake_A_freq[0])

		# Total loss
		loss_G_A2B = loss_GAN_A2B + 5 * loss_pix_A2B # + freq_weight * (loss_freqH_A2B)# + loss_freqL_A2B)
		loss_G_B2A = loss_GAN_B2A + 5 * loss_pix_B2A # + freq_weight * (loss_freqH_B2A)# + loss_freqL_B2A)
		...
		# Grad_penalty
		# grad_penalty_A = gradient_penalty(netD_A, real_A, fake_A.detach(), opt.batchSize)

		# Total loss
		loss_D_A = (loss_D_real + loss_D_fake) # + grad_penalty_A
		loss_D_A.backward()
		...
		# Grad_penalty
		# grad_penalty_B = gradient_penalty(netD_B, real_B, fake_B.detach(), opt.batchSize)

		# Total loss
		loss_D_B = (loss_D_real + loss_D_fake) # + grad_penalty_B
		loss_D_B.backward()
		...
		if local_rank == 0:
			logger.log({
			    ...
			    'loss_G_pix': (loss_pix_A2B + loss_pix_B2A),
			    'loss_D': (loss_D_B + loss_D_A),
				# 'loss_freq': (loss_freqH_A2B + loss_freqH_B2A)#+ loss_freqL_B2A+loss_freqL_A2B),
			            }
			)
	...
	if local_rank == 0:
		torch.save(netG_A2B.module.state_dict(), './output/' + str(epoch) + '_netG_A2B' + '.pth')
		torch.save(netG_B2A.module.state_dict(), './output/' + str(epoch) + '_netG_B2A' + '.pth')
		torch.save(netD_A.module.state_dict(), './output/' + str(epoch) + '_netD_A' + '.pth')
		torch.save(netD_B.module.state_dict(), './output/' + str(epoch) + '_netD_B' + '.pth')
