...
def train_syndiff(rank, gpu, args):
    ...
    gen_diffusive_2.load_state_dict(
        torch.load(os.path.join(exp_path, 'gen_diffusive_2_{}.pth'.format(480)), map_location=device))
    gen_non_diffusive_1to2.load_state_dict(
        torch.load(os.path.join(exp_path, 'gen_non_diffusive_1to2_{}.pth'.format(480)), map_location=device))
    gen_non_diffusive_2to1.load_state_dict(
        torch.load(os.path.join(exp_path, 'gen_non_diffusive_2to1_{}.pth'.format(480)), map_location=device))
    # disc_diffusive_1.load_state_dict(torch.load('', map_location=device))
    disc_diffusive_2.load_state_dict(
        torch.load(os.path.join(exp_path, 'disc_diffusive_2_{}.pth'.format(480)), map_location=device))
    disc_non_diffusive_cycle1.load_state_dict(
        torch.load(os.path.join(exp_path, 'disc_non_diffusive_cycle1_{}.pth'.format(480)), map_location=device))
    disc_non_diffusive_cycle2.load_state_dict(
        torch.load(os.path.join(exp_path, 'disc_non_diffusive_cycle2_{}.pth'.format(480)), map_location=device))
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            ...
            mask_H, mask_L = make_mask_(size, distance, 5, 5)
            real_MR_freq = Fourier_trans(real_data2, mask_H, mask_L)
            real_CT_freq = Fourier_trans(real_data1, mask_H, mask_L)
            ...
            #frequency loss
            fake_MR_freq = Fourier_trans(x2_0_predict, mask_H, mask_L)
            fake_CT_freq = Fourier_trans(x1_0_predict, mask_H, mask_L)
            # fake_diff_MR_freq = Fourier_trans(x2_0_predict_diff[:,[0],:], mask_H, mask_L)

            errG_freq = F.l1_loss(real_CT_freq[0], fake_MR_freq[0]) + F.l1_loss(real_MR_freq[0], fake_CT_freq[0]) # \
                        # + F.l1_loss(fake_diff_MR_freq[0], fake_CT_freq[0])
        ...
        #         torchvision.utils.save_image(x2_pos_sample, os.path.join(exp_path, 'xpos2_epoch_{}.png'.format(epoch)), normalize=True)
            ...
            if epoch % args.save_ckpt_every == 0:
                ...
                torch.save(gen_diffusive_2.state_dict(), os.path.join(exp_path, 'gen_diffusive_2_{}.pth'.format(epoch + 481)))
                torch.save(gen_non_diffusive_1to2.state_dict(), os.path.join(exp_path, 'gen_non_diffusive_1to2_{}.pth'.format(epoch + 481)))
                torch.save(gen_non_diffusive_2to1.state_dict(), os.path.join(exp_path, 'gen_non_diffusive_2to1_{}.pth'.format(epoch + 481)))

                torch.save(disc_diffusive_2.state_dict(), os.path.join(exp_path, 'disc_diffusive_2_{}.pth'.format(epoch + 481)))
                torch.save(disc_non_diffusive_cycle1.state_dict(), os.path.join(exp_path, 'disc_non_diffusive_cycle1_{}.pth'.format(epoch + 481)))
                torch.save(disc_non_diffusive_cycle2.state_dict(), os.path.join(exp_path, 'disc_non_diffusive_cycle2_{}.pth'.format(epoch + 481)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_process_per_node', type=int, default=4, help='number of gpus')
