...
def sample_and_test(args):
    ...
    save_dir = exp_path + "/generated_samples/epoch_{}".format(100)  # 固定路径名字
    ...
    for iteration, (x , y) in enumerate(data_loader): 
        ...
        real_data = y.to(device, non_blocking=True)  # MR
        source_data = x.to(device, non_blocking=True)  # CT
        ...
        torchvision.utils.save_image(fake_sample2.permute(0, 1, 3, 2), '{}/{}_samples2_{}.jpg'.format(save_dir, phase, iteration), normalize=True)
        # torchvision.utils.save_image(source_data.permute(0, 1, 3, 2), '{}/{}_samples2_{}.jpg'.format(save_dir, phase, iteration), normalize=True)
    #
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ch_mult', nargs='+', default=[1, 1, 2, 2, 4, 4], type=int,
    ...
    parser.add_argument('--which_epoch', type=int, default=95)
    parser.add_argument('--gpu_chose', type=int, default=0)


    parser.add_argument('--source', type=str, default='CT', help='source contrast')
    args = parser.parse_args()
    FID = []

    # for test_epoch in range(481, 632, 10):
    test_epoch = 631
    args.which_epoch = test_epoch
    sample_and_test(args)



    fid = FID_score('./dataset/MR/', './output/exp_syndiff/generated_samples/epoch_100/')
    print('epoch-{} | FID-{}'.format(test_epoch, fid))
    FID.append(fid)
