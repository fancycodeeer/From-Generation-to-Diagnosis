...
import torchvision
from utils.utils import Fourier_trans, make_mask_, make_distance_mask_
import torchvision.transforms as transforms
from dataset import CreateDatasetSynthesis, RandomPairDataset
...
def train_syndiff(rank, gpu, args):
    ...
    from backbones.threshold_predictor import CrossModalAttentionNetwork
    ...
    dataset = RandomPairDataset(phase="train", input_path=args.input_path, contrast1=args.contrast1, contrast2=args.contrast2)
    # dataset = CreateDatasetSynthesis(phase = "train", input_path = args.input_path, contrast1 = args.contrast1, contrast2 = args.contrast2)
    # dataset_val = CreateDatasetSynthesis(phase = "test", input_path = args.input_path, contrast1 = args.contrast1, contrast2 = args.contrast2 )
    ...
    size = [args.batch_size, 1, args.image_size, int(args.image_size / 2 + 1)]
    distance = make_distance_mask_(size)
    ...
    # 特征融合网络
    selector_freq_nodiff_1 = CrossModalAttentionNetwork().to(device)
    selector_freq_nodiff_2 = CrossModalAttentionNetwork().to(device)
    selector_freq_diff = CrossModalAttentionNetwork().to(device)

    #networks performing reverse denoising
    # gen_diffusive_1 = NCSNpp(args).to(device)
    gen_diffusive_2 = NCSNpp(args).to(device)
    # disc_diffusive_1 = Discriminator_small(nc = 2, ngf = args.ngf,
    #                                t_emb_dim = args.t_emb_dim,
    #                                act=nn.LeakyReLU(0.2)).to(device)
    disc_diffusive_2 = Discriminator_small(nc = 2, ngf = args.ngf,
                                   t_emb_dim = args.t_emb_dim,
                                   act=nn.LeakyReLU(0.2)).to(device)
    ...
    # broadcast_params(selector_freq.parameters())
    broadcast_params(selector_freq_nodiff_1.parameters())
    broadcast_params(selector_freq_nodiff_2.parameters())
    broadcast_params(selector_freq_diff.parameters())
    ...
    optimizer_selector_nodiffusive_1 = optim.Adam(selector_freq_nodiff_1.parameters(), lr=args.lr_g, betas=(args.beta1, args.beta2))
    optimizer_selector_nodiffusive_2 = optim.Adam(selector_freq_nodiff_2.parameters(), lr=args.lr_g, betas=(args.beta1, args.beta2))
    optimizer_selector_diffusive = optim.Adam(selector_freq_diff.parameters(), lr=args.lr_g, betas=(args.beta1, args.beta2))
    ...
    scheduler_selector_nodiffusive_1 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_selector_nodiffusive_1, args.num_epoch, eta_min=1e-5)
    scheduler_selector_nodiffusive_2 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_selector_nodiffusive_2, args.num_epoch, eta_min=1e-5)
    scheduler_selector_diffusive = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_selector_diffusive, args.num_epoch, eta_min=1e-5)
    ...
    selector_freq_nodiff_1 =nn.parallel.DistributedDataParallel(selector_freq_nodiff_1, device_ids=[gpu])
    selector_freq_nodiff_2 = nn.parallel.DistributedDataParallel(selector_freq_nodiff_2, device_ids=[gpu])
    selector_freq_diff = nn.parallel.DistributedDataParallel(selector_freq_diff, device_ids=[gpu])

    # 加载预训练
    gen_diffusive_2.load_state_dict(
        torch.load(os.path.join(exp_path, 'gen_diffusive_2_{}.pth'.format(495)), map_location=device))
    gen_non_diffusive_1to2.load_state_dict(
        torch.load(os.path.join(exp_path, 'gen_non_diffusive_1to2_{}.pth'.format(495)), map_location=device))
    gen_non_diffusive_2to1.load_state_dict(
        torch.load(os.path.join(exp_path, 'gen_non_diffusive_2to1_{}.pth'.format(495)), map_location=device))
    # disc_diffusive_1.load_state_dict(torch.load('', map_location=device))
    disc_diffusive_2.load_state_dict(
        torch.load(os.path.join(exp_path, 'disc_diffusive_2_{}.pth'.format(495)), map_location=device))
    disc_non_diffusive_cycle1.load_state_dict(
        torch.load(os.path.join(exp_path, 'disc_non_diffusive_cycle1_{}.pth'.format(495)), map_location=device))
    disc_non_diffusive_cycle2.load_state_dict(
        torch.load(os.path.join(exp_path, 'disc_non_diffusive_cycle2_{}.pth'.format(495)), map_location=device))

    selector_freq_nodiff_1.load_state_dict(
        torch.load(os.path.join(exp_path, 'selector_nodiff_1_{}.pth'.format(495)), map_location=device))
    selector_freq_nodiff_2.load_state_dict(
        torch.load(os.path.join(exp_path, 'selector_nodiff_2_{}.pth'.format(495)), map_location=device))
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            ...
            #sample from p(x_0)
            real_data1 = x1.to(device, non_blocking=True)  # CT
            real_data2 = x2.to(device, non_blocking=True)  # MR
            ...
            errD_real = errD2_real # + errD1_real
            ...
            if args.lazy_reg is None:
                ...
                grad_penalty = args.r1_gamma / 2 * grad2_penalty  # + args.r1_gamma / 2 * grad1_penalty
            ...
            errD_fake = errD2_fake.mean()  # + errD1_fake.mean()
            errD_fake.backward()    
            ...
            selector_freq_diff.zero_grad()
            selector_freq_nodiff_1.zero_grad()
            selector_freq_nodiff_2.zero_grad()
            ...
            # FDDT loss
            D_h_real_CT, D_l_real_CT = selector_freq_nodiff_1(real_data1)
            mask_H_real_CT, mask_L_real_CT = make_mask_(size, distance, D_h_real_CT, D_l_real_CT)
            real_CT_freq_0, real_CT_freq_1 = Fourier_trans(real_data1, mask_H_real_CT, mask_L_real_CT)
            D_h_fake_MR, D_l_fake_MR = selector_freq_nodiff_1(x2_0_predict)
            mask_H_fake_MR, mask_L_fake_MR = make_mask_(size, distance, D_h_fake_MR, D_l_fake_MR)
            fake_MR_freq_0, fake_MR_freq_1 = Fourier_trans(x2_0_predict, mask_H_fake_MR, mask_L_fake_MR)
            loss_freq_1 = F.l1_loss(gen_non_diffusive_1to2(real_CT_freq_0), fake_MR_freq_0) + F.l1_loss(gen_non_diffusive_1to2(real_CT_freq_1), fake_MR_freq_1)

            D_h_real_MR, D_l_real_MR = selector_freq_nodiff_2(real_data2)
            mask_H_real_MR, mask_L_real_MR = make_mask_(size, distance, D_h_real_MR, D_l_real_MR)
            real_MR_freq_0, real_MR_freq_1 = Fourier_trans(real_data2, mask_H_real_MR, mask_L_real_MR)
            D_h_fake_CT, D_l_fake_CT = selector_freq_nodiff_2(x1_0_predict)
            mask_H_fake_CT, mask_L_fake_CT = make_mask_(size, distance, D_h_fake_CT, D_l_fake_CT)
            fake_CT_freq_0, fake_CT_freq_1 = Fourier_trans(x1_0_predict, mask_H_fake_CT, mask_L_fake_CT)
            loss_freq_2 = F.l1_loss(gen_non_diffusive_2to1(real_MR_freq_0), fake_CT_freq_0) + F.l1_loss(gen_non_diffusive_2to1(real_MR_freq_1), fake_CT_freq_1)

            # D_h_diff_MR, D_l_diff_MR = selector_freq_diff(x2_0_predict_diff[:,[0],:])
            # mask_H_diff_MR, mask_L_diff_MR = make_mask_(size, distance, D_h_diff_MR, D_l_diff_MR)
            # diff_MR_freq_0, diff_MR_freq_1 = Fourier_trans(x2_0_predict_diff[:,[0],:], mask_H_diff_MR, mask_L_diff_MR)
            # loss_freq_3 = F.l1_loss(gen_diffusive_2(torch.cat((x2_tp1.detach(),fake_CT_freq_0),axis=1), t2, latent_z2)[:,[0],:], diff_MR_freq_0) \
            #               + F.l1_loss(gen_diffusive_2(torch.cat((x2_tp1.detach(),fake_CT_freq_1),axis=1), t2, latent_z2)[:,[0],:], diff_MR_freq_1)


            loss_freq = loss_freq_1 + loss_freq_2 # + loss_freq_3

            #L1 loss
            # errG1_L1 = F.l1_loss(x1_0_predict_diff[:,[0],:],real_data1)
            errG2_L1 = F.l1_loss(x2_0_predict_diff[:,[0],:],real_data2)
            errG_L1 = errG2_L1 # + errG1_L1
            ...
            errG = args.lambda_l1_loss*errG_cycle +  errG_adv + errG_cycle_adv + args.lambda_l1_loss*errG_L1 + loss_freq
            errG.backward()
            ...
            optimizer_selector_diffusive.step()
            optimizer_selector_nodiffusive_1.step()
            optimizer_selector_nodiffusive_2.step()

            # for name, param in selector_freq_nodiff_1.named_parameters():
            #     if param.grad is None:
            #         print(f"⚠️ 未使用的参数: {name}")
        ...
        if not args.no_lr_decay:
            ...
            scheduler_selector_diffusive.step()
            scheduler_selector_nodiffusive_1.step()
            scheduler_selector_nodiffusive_2.step()
            ...
            if epoch % args.save_ckpt_every == 0:
                ...
                torch.save(gen_diffusive_2.state_dict(), os.path.join(exp_path, 'gen_diffusive_2_{}.pth'.format(epoch + 496)))
                torch.save(gen_non_diffusive_1to2.state_dict(), os.path.join(exp_path, 'gen_non_diffusive_1to2_{}.pth'.format(epoch + 496)))
                torch.save(gen_non_diffusive_2to1.state_dict(), os.path.join(exp_path, 'gen_non_diffusive_2to1_{}.pth'.format(epoch + 496)))

                torch.save(disc_diffusive_2.state_dict(), os.path.join(exp_path, 'disc_diffusive_2_{}.pth'.format(epoch + 496)))
                torch.save(disc_non_diffusive_cycle1.state_dict(), os.path.join(exp_path, 'disc_non_diffusive_cycle1_{}.pth'.format(epoch + 496)))
                torch.save(disc_non_diffusive_cycle2.state_dict(), os.path.join(exp_path, 'disc_non_diffusive_cycle2_{}.pth'.format(epoch + 496)))

                # torch.save(selector_freq_diff.state_dict(),os.path.join(exp_path, 'selector_diff_{}.pth'.format(epoch + 496)))
                torch.save(selector_freq_nodiff_1.state_dict(),os.path.join(exp_path, 'selector_nodiff_1_{}.pth'.format(epoch + 496)))
                torch.save(selector_freq_nodiff_2.state_dict(),os.path.join(exp_path, 'selector_nodiff_2_{}.pth'.format(epoch + 496)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp', default='exp_syndiff_FDDT', help='name of experiment')
    parser.add_argument('--input_path', default='./dataset/', help='path to input data')
    ...
    parser.add_argument('--lr_g', type=float, default=3e-5, help='learning rate g')  # 1.6e-4
    parser.add_argument('--lr_d', type=float, default=2e-5, help='learning rate d')  # 1e-4
    ...
    parser.add_argument('--contrast1', type=str, default='CT', help='contrast selection for model')
    parser.add_argument('--contrast2', type=str, default='MR',help='contrast selection for model')
    parser.add_argument('--port_num', type=str, default='6021', help='port selection for code')

   
    args = parser.parse_args()
    args.world_size = args.num_proc_node * args.num_process_per_node
    mp.set_start_method('spawn', force=True)  # 设置 spawn 作为启动方法，而不是默认的 fork
    size = args.num_process_per_node  # 一个机子上的GPU数量
