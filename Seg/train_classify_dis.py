...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for index, (t1_1_image, t1ce_1_image, t1_2_image, t1ce_2_image) in enumerate(train_loader):
            t1_1_image = t1_1_image.cuda()
            t1ce_1_image = t1ce_1_image.cuda()
            t1_2_image = t1_2_image.cuda()
            ...
            outputs = model(t1_1_image)
            outputs_dis = model_t(translator(t1_1_image))

            _, predicted = torch.max(outputs, 1)
            correct_train += (predicted == label_0).sum().item()
            total_train += label_0.size(0)

            loss_1 = CELoss(outputs, label_0) + CELoss(outputs_dis, label_0)
            if CELoss(outputs, label_0) >= CELoss(outputs_dis, label_0):
                loss_distill_1 = utils.Distillation_loss(features_s, features_t)
            else:
                loss_distill_1 = 0.0
            loss_1 = loss_1 + loss_distill_1
            loss_1.backward()
            optimizer.step()
            optimizer_g.step()
            optimizer_t.step()
            ...
            outputs = model(t1_2_image)
            outputs_dis = model_t(translator(t1_2_image))

            _, predicted = torch.max(outputs, 1)
            correct_train += (predicted == label_1).sum().item()
            total_train += label_1.size(0)

            loss_2 = CELoss(outputs, label_1) + CELoss(outputs_dis, label_1)
            if CELoss(outputs, label_1) >= CELoss(outputs_dis, label_1):
                loss_distill_2 = utils.Distillation_loss(features_s, features_t)
            else:
                loss_distill_2 = 0.0
            loss_2 = loss_2 + loss_distill_2
            loss_2.backward()
            optimizer.step()
            optimizer_g.step()
            optimizer_t.step()
            ...
            # 微调GAN
            optimizer_g.zero_grad()
            fake_T1ce_gan = [translator(t1_1_image), translator(t1_2_image)]
            instance_loss = F.mse_loss(fake_T1ce_gan[0], t1ce_1_image) + F.mse_loss(fake_T1ce_gan[1], t1ce_2_image)
            # pre_fake_B = discriminator_B(fake_T1ce_gan)
            # loss_gan = BCELoss(pre_fake_B, torch.ones_like(pre_fake_B))

            loss_g = instance_loss
            loss_g.backward()
            running_loss_g = running_loss_g + loss_g.item() * t1_1_image.size(0)
            optimizer_g.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints/classify_dis")
