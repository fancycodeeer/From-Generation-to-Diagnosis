...
class CrossModalAttentionNetwork(nn.Module):
	def __init__(self, threshold_minmax=[0.5, 4.5, 5, 25], num_heads=2, in_channels=1):
		...
		self.frequency_extractor = nn.Sequential(
			nn.Conv2d(self.in_channels * 2, 16, kernel_size=3, padding=1),
			nn.InstanceNorm2d(16),
			nn.AvgPool2d(2),
			nn.ReLU(),
			nn.Conv2d(16, 32, kernel_size=3, padding=1),
...
# x = torch.randn(2, 1, 240, 240).cuda()
# model = CrossModalAttentionNetwork().cuda()
# h, l = model(x)
#
# print(h.view(-1, 1, 1, 1).shape)
# print(l)