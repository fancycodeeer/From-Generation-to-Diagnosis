...
def main():
    ...
    if is_main(rank):
        ...
        print(f'[INFO] eval policy: every epoch evaluates val and test; train split is not evaluated.')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        test_eval = evaluate_split(model, test_loader, criterion, device, epoch, 'test', args.out_dir, args, rank, is_ddp)
        stop_now = False
        if is_main(rank):
            ...
            print(f"Epoch {epoch:03d} | train_loss={fmt_float(row.get('train_step_loss', np.nan), 5)} | val_loss={fmt_float(row.get('val_loss', np.nan), 5)} | test_loss={fmt_float(row.get('test_loss', np.nan), 5)} | {monitor_status}")
    ...
    for monitor in args.monitor_metrics:
        ...
        if not exists:
            if is_main(rank):
                print(f'[WARNING] {best_path} not found; final test skipped for monitor={monitor}.')
            continue
        load_model_only(best_path, model, device)
        split_name = f'test_best_{safe_name}'
        test_summary = evaluate_split(model, test_loader, criterion, device, int(best_epochs.get(monitor, 0)), split_name, args.out_dir, args, rank, is_ddp)
        if is_main(rank):
            row = {'monitor': monitor, 'checkpoint': str(best_path), 'best_epoch': int(best_epochs.get(monitor, 0)), 'best_val_value': float(best_metrics.get(monitor, np.nan))}
            ...
            print(f'[FINAL TEST] monitor={monitor}, loaded {best_path.name}, epoch={best_epochs.get(monitor, 0)}')
