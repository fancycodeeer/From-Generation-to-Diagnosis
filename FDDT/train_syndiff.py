...
from models_syndiff import SynDiffCoreBundle, apply_ema_to_generators, build_ema_helpers, build_syndiff_core_models, ddp_wrap_models, load_model_state, model_state_dict, restore_generators_from_ema, set_requires_grad, update_ema_helpers, unwrap_model
from utils_ import append_csv, is_main_process, psnr_per_image, seed_everything, seed_worker, ssim_per_image, tensor_to_01
...
def parse_args():
    parser = argparse.ArgumentParser('SynDiff-core, stable AMP, adapted to your project')
...
def all_finite(loss_dict: Dict[str, torch.Tensor]) -> Tuple[bool, str]:
...
def zero_optimizers(optimizers: Dict[str, torch.optim.Optimizer]) -> None:
...
class Diffusion_Coefficients:
    ...
    def __init__(self, args, device):
        ...
        a_s_cum = np.cumprod(self.a_s.detach().cpu())
        self.a_s_cum = a_s_cum.to(device=device, dtype=torch.float32)
        self.sigmas_cum = torch.sqrt(1.0 - self.a_s_cum ** 2)
        self.a_s_prev = self.a_s.clone()
        self.a_s_prev[-1] = 1.0

def q_sample(coeff: Diffusion_Coefficients, x_start, t, noise=None):
    if noise is None:
        noise = torch.randn_like(x_start)
    return extract(coeff.a_s_cum, t, x_start.shape) * x_start + extract(coeff.sigmas_cum, t, x_start.shape) * noise

def q_sample_pairs(coeff: Diffusion_Coefficients, x_start, t):
    noise = torch.randn_like(x_start)
    x_t = q_sample(coeff, x_start, t, noise=noise)
...
def compute_case_metrics_from_slice_entries(slice_entries: List[Dict]) -> Tuple[List[Dict], Dict]:
    ...
    for item in slice_entries:
        unique.setdefault(item['slice_id'], item)
...
def save_case_metrics_to_xlsx(xlsx_path: str, epoch: int, case_rows: List[Dict], summary: Dict):
    ...
    def remove_epoch_rows(ws):
        rows = [r for r in range(2, ws.max_row + 1) if ws.cell(r, 1).value == epoch]
...
def validate_A_to_B_case_level(models: SynDiffCoreBundle, val_loader, device, args, pos_coeff, ema_helpers=None):
    ...
    torch.manual_seed(args.val_seed + rank)
    if torch.cuda.is_available():
        torch.cuda.manual_seed(args.val_seed + rank)
    local_entries = []
    for batch in val_loader:
        ...
        ids = batch.get('key', batch.get('A_path', [str(i) for i in range(len(ssim_vals))]))
        for i in range(len(ssim_vals)):
            sid = ids[i]
            local_entries.append({'case_id': case_id_from_any_path_or_key(sid, args.case_id_regex), 'slice_id': slice_id_from_any_path_or_key(sid), 'ssim': float(ssim_vals[i]), 'psnr': float(psnr_vals[i])})
...
def save_full_checkpoint(path: str, epoch: int, global_step: int, models: SynDiffCoreBundle, optimizers: Dict, schedulers: Dict, scaler: GradScaler, args, best: Dict, ema_helpers: Dict):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    torch.save({'epoch': epoch, 'global_step': global_step, 'args': vars(args), 'best': best, 'gen_diffusive_1': model_state_dict(models.gen_diffusive_1), 'gen_diffusive_2': model_state_dict(models.gen_diffusive_2), 'gen_non_diffusive_1to2': model_state_dict(models.gen_non_diffusive_1to2), 'gen_non_diffusive_2to1': model_state_dict(models.gen_non_diffusive_2to1), 'disc_diffusive_1': model_state_dict(models.disc_diffusive_1), 'disc_diffusive_2': model_state_dict(models.disc_diffusive_2), 'disc_cycle_1': model_state_dict(models.disc_cycle_1), 'disc_cycle_2': model_state_dict(models.disc_cycle_2), 'optimizers': {k: v.state_dict() for k, v in optimizers.items()}, 'schedulers': {k: v.state_dict() for k, v in schedulers.items()}, 'scaler': scaler.state_dict(), 'ema': {k: v.state_dict() for k, v in ema_helpers.items()} if ema_helpers else {}}, path)

def load_full_checkpoint(path: str, models, optimizers, schedulers, scaler, ema_helpers, device):
    ckpt = torch.load(path, map_location=device)
    load_model_state(models.gen_diffusive_1, ckpt['gen_diffusive_1'])
    load_model_state(models.gen_diffusive_2, ckpt['gen_diffusive_2'])
    load_model_state(models.gen_non_diffusive_1to2, ckpt['gen_non_diffusive_1to2'])
    load_model_state(models.gen_non_diffusive_2to1, ckpt['gen_non_diffusive_2to1'])
    load_model_state(models.disc_diffusive_1, ckpt['disc_diffusive_1'])
    load_model_state(models.disc_diffusive_2, ckpt['disc_diffusive_2'])
    load_model_state(models.disc_cycle_1, ckpt['disc_cycle_1'])
    load_model_state(models.disc_cycle_2, ckpt['disc_cycle_2'])
    ...
    for k, sch in schedulers.items():
        if k in ckpt.get('schedulers', {}):
            sch.load_state_dict(ckpt['schedulers'][k])
    if 'scaler' in ckpt:
        scaler.load_state_dict(ckpt['scaler'])
    if ema_helpers and 'ema' in ckpt:
        for k, helper in ema_helpers.items():
            helper.load_state_dict(ckpt['ema'].get(k), device=device)
    return ckpt

def save_A_to_B_weights(path: str, models: SynDiffCoreBundle):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    torch.save(model_state_dict(models.gen_diffusive_2), path)
...
def main():
    ...
    if is_main_process():
        ...
        print(f'[Data] train A: {args.dataroot}/train/{args.mode}')
        print(f'[Data] val A:   {args.dataroot}/val/{args.mode}')
    ...
    train_set = UnpairedBratsDataset(args.dataroot, 'train', train_tf, args.mode)
    val_set = PairedBratsValDataset(args.dataroot, 'val', val_tf, args.mode)
    ...
    optimizers = {'gen_diffusive_1': torch.optim.Adam(models.gen_diffusive_1.parameters(), lr=args.lr_g, betas=(args.beta1, args.beta2)), 'gen_diffusive_2': torch.optim.Adam(models.gen_diffusive_2.parameters(), lr=args.lr_g, betas=(args.beta1, args.beta2)), 'gen_non_diffusive_1to2': torch.optim.Adam(models.gen_non_diffusive_1to2.parameters(), lr=args.lr_g, betas=(args.beta1, args.beta2)), 'gen_non_diffusive_2to1': torch.optim.Adam(models.gen_non_diffusive_2to1.parameters(), lr=args.lr_g, betas=(args.beta1, args.beta2)), 'disc_diffusive_1': torch.optim.Adam(models.disc_diffusive_1.parameters(), lr=args.lr_d, betas=(args.beta1, args.beta2)), 'disc_diffusive_2': torch.optim.Adam(models.disc_diffusive_2.parameters(), lr=args.lr_d, betas=(args.beta1, args.beta2)), 'disc_cycle_1': torch.optim.Adam(models.disc_cycle_1.parameters(), lr=args.lr_d, betas=(args.beta1, args.beta2)), 'disc_cycle_2': torch.optim.Adam(models.disc_cycle_2.parameters(), lr=args.lr_d, betas=(args.beta1, args.beta2))}
    schedulers = {}
    ...
    try:
        for epoch in range(start_epoch, args.num_epoch + 1):
            ...
            for net in [models.gen_diffusive_1, models.gen_diffusive_2, models.gen_non_diffusive_1to2, models.gen_non_diffusive_2to1, models.disc_diffusive_1, models.disc_diffusive_2, models.disc_cycle_1, models.disc_cycle_2]:
                net.train()
            loss_acc = {'G_sum': 0.0, 'G_cycle': 0.0, 'G_L1': 0.0, 'G_adv': 0.0, 'G_cycle_adv': 0.0, 'D_diff': 0.0, 'D_cycle': 0.0, 'R1': 0.0}
            n_iter = 0
            for batch in train_loader:
                real_data1 = batch['A'].to(device, non_blocking=True)
                real_data2 = batch['B'].to(device, non_blocking=True)
                bsz = real_data1.size(0)
                r1_value = torch.zeros((), device=device, dtype=torch.float32)
                ...
                for k in ['gen_diffusive_1', 'gen_diffusive_2', 'gen_non_diffusive_1to2', 'gen_non_diffusive_2to1']:
                    optimizers[k].zero_grad(set_to_none=True)
                t1 = torch.randint(0, args.num_timesteps, (bsz,), device=device)
                t2 = torch.randint(0, args.num_timesteps, (bsz,), device=device)
                x1_t, x1_tp1 = q_sample_pairs(coeff, real_data1, t1)
                ...
                ok, bad_name = all_finite({'G_sum': errG, 'G_cycle': errG_cycle, 'G_L1': errG_L1, 'G_adv': errG_adv, 'G_cycle_adv': errG_cycle_adv})
                if not ok:
                    nan_batches += 1
                    zero_optimizers(optimizers)
                    if is_main_process():
            ...
            if is_main_process():
                ...
                save_full_checkpoint(os.path.join(exp_dir, 'latest.pth'), epoch, global_step, models, optimizers, schedulers, scaler, args, best, ema_helpers)
                if epoch % args.save_content_every == 0:
                    save_full_checkpoint(os.path.join(exp_dir, f'content_epoch_{epoch:03d}.pth'), epoch, global_step, models, optimizers, schedulers, scaler, args, best, ema_helpers)
                if args.save_epoch_weights:
                    save_A_to_B_weights(os.path.join(exp_dir, f'netG_A2B_epoch_{epoch:03d}.pth'), models)
                print(f"[Epoch {epoch:03d}/{args.num_epoch}] lr_g={lr_g:.2e} lr_d={lr_d:.2e} G={avg['G_sum']:.4f} Gcyc={avg['G_cycle']:.4f} GL1={avg['G_L1']:.4f} Gadv={avg['G_adv']:.4f} GcycAdv={avg['G_cycle_adv']:.4f} D={avg['D_diff']:.4f} Dcyc={avg['D_cycle']:.4f} R1={avg['R1']:.4f} SSIM={cur_ssim:.5f}±{summary['case_ssim_std']:.5f} PSNR={cur_psnr:.3f}±{summary['case_psnr_std']:.3f} best_ssim={best['best_ssim']:.5f}@{best['best_ssim_epoch']} best_psnr={best['best_psnr']:.3f}@{best['best_psnr_epoch']} patience={best['bad_epochs']}/{args.patience} time={(time.time() - epoch_start) / 60.0:.1f}m")
            if is_main_process():
